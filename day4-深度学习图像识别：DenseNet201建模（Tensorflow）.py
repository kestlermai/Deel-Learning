# -*- coding: utf-8 -*-
"""
Created on Wed Oct 18 19:38:38 2023

@author: maihuanzhuo
"""

#深度学习图像识别：DenseNet201建模（Tensorflow）

'''
DenseNet201是一种深度卷积神经网络，是DenseNet网络的一种变体。
DenseNet，全称Dense Convolutional Network（密集卷积网络），是由Facebook AI Research在2016年提出的一种卷积神经网络架构。

DenseNet的主要特点是其密集连接的特性。在DenseNet中，每个层都与之前所有层直接连接，每一层的输入不仅包括前一层的输出，而且还包括所有之前层的输出。
这种结构可以增强特征的传播，使得网络可以利用所有层的特征信息，有助于改善梯度流和稀疏性学习。

DenseNet201是DenseNet系列中的一种，其结构比DenseNet121更深，拥有201层。
这种深度使得DenseNet201在处理图像分类任务时，能够获得更好的性能。
其主要优势在于其能够有效地利用特征，减少参数数量，降低过拟合的风险，提高模型的泛化能力。
同时，由于其复杂的网络结构，DenseNet201的训练需要较大的计算资源。
'''
...
#https://blog.csdn.net/weixin_40459958/article/details/124452966，这里比较直观对比
...
base_model = DenseNet201(include_top=False, #是否包含顶层的全连接层
...
x = Dropout(0.8)(x)
#Dense
x = Dense(128, kernel_regularizer=regularizers.l2(0.1))(x)  #添加L2正则化
#BatchNormalization
x = BatchNormalization()(x)
#激活函数
x = Activation('relu')(x)
...
# _________________________________________________________________
# batch_normalization_191 (Bat (None, 1920)              7680      
# _________________________________________________________________
...
# _________________________________________________________________
# batch_normalization_193 (Bat (None, 2)                 8         
# _________________________________________________________________
# activation_191 (Activation)  (None, 2)                 0         
# =================================================================
# Total params: 18,576,330
# Trainable params: 18,343,174
# Non-trainable params: 233,156
# _________________________________________________________________
# None
...
# 设置动态学习率
annealer = LearningRateScheduler(lambda x: 1e-4 * 0.99 ** (x+NO_EPOCHS))
...
# 设置模型检查点
checkpointer = ModelCheckpoint('./day4-TensorFlow_DenseNet_MTB/mtb_best_model_DenseNet201.h5',
                                monitor='val_accuracy',
                                verbose=VERBOSE,
                                save_best_only=True,
                                save_weights_only=True,
                                mode='max')#监测的指标应当越大越好
...
#保存模型
model.save('./day4-TensorFlow_DenseNet_MTB/mtb_best_model_DenseNet201.h5')
